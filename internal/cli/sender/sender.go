...
func Run(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		if (arg == "--chunk-size" || arg == "--total-streams") && i+1 < len(args) {
			i++
			value := args[i]
			switch arg {
			case "--chunk-size":
...
func printSenderUsage() {
	fmt.Fprintln(termio.StderrFile(), "usage: thru host <paths...> [--max-receivers N] [--server-url URL] [--benchmark]")
	fmt.Fprintln(termio.StderrFile(), "  --max-receivers N           max concurrent receivers (default 4)")
	fmt.Fprintln(termio.StderrFile(), "  --server-url URL            signaling server URL (default https://bytepipe.app)")
	fmt.Fprintf(termio.StderrFile(), "  --stun-server URLS          STUN server URLs (comma-separated, default %s)\n", strings.Join(senderDefaultStunServers, ","))
	fmt.Fprintln(termio.StderrFile(), "                              example: --stun-server stun:stun.l.google.com:19302")
	fmt.Fprintln(termio.StderrFile(), "  --turn-server URLS          TURN server URLs (comma-separated, default none; server may provide)")
	fmt.Fprintln(termio.StderrFile(), "                              example: --turn-server turn:username:password@turn.example.com:3478")
	fmt.Fprintln(termio.StderrFile(), "                                       --turn-server turns:username:password@turn.example.com:5349")
	fmt.Fprintln(termio.StderrFile(), "                                       --turn-server turns:username:password@turn.example.com:5349?servername=turn.example.com")
	fmt.Fprintln(termio.StderrFile(), "                                       --turn-server turns:username:password@turn.example.com:5349?insecure=1  (debug only)")
	fmt.Fprintln(termio.StderrFile(), "  --test-turn                 only use TURN relay candidates (no direct probing)")
	fmt.Fprintln(termio.StderrFile(), "  --benchmark                 enable benchmark stats")
	fmt.Fprintln(termio.StderrFile(), "  --verbose                   enable verbose UI/logging")
	fmt.Fprintln(termio.StderrFile(), "  --no-resume                 disable resume handshake")
	fmt.Fprintln(termio.StderrFile(), "  --total-connections N       total QUIC connections (default 4)")
	fmt.Fprintln(termio.StderrFile(), "  --udp-read-buffer-bytes N   UDP read buffer size (default 8388608)")
	fmt.Fprintln(termio.StderrFile(), "  --udp-write-buffer-bytes N  UDP write buffer size (default 8388608)")
	fmt.Fprintln(termio.StderrFile(), "  --quic-conn-window-bytes N  QUIC connection window (default 536870912)")
	fmt.Fprintln(termio.StderrFile(), "  --quic-stream-window-bytes N QUIC stream window (default 67108864)")
	fmt.Fprintln(termio.StderrFile(), "  --quic-max-incoming-streams N max QUIC incoming streams (default 256)")
	fmt.Fprintln(termio.StderrFile(), "  --chunk-size N              chunk size in bytes (default 0=auto)")
	fmt.Fprintln(termio.StderrFile(), "  --total-streams N           total concurrent transfer streams (default 64, 1..128)")
}

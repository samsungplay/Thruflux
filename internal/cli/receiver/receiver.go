...
func printReceiverUsage() {
	...
	fmt.Fprintln(os.Stderr, "  --yes                       auto-accept transfer prompts")
	fmt.Fprintln(os.Stderr, "  --no-resume                 overwrite existing data without prompting")
	fmt.Fprintln(os.Stderr, "  --yes-resume                resume existing data without prompting")
	...
	fmt.Fprintln(os.Stderr, "  --quic-max-incoming-streams N max QUIC incoming streams (default 256)")
}
